...
public:
			...
			bool is_highlevel)
	{
		...
		video::SMaterial texture=m_sky->getMaterial(0);
		int layer = 3;
		services->setPixelShaderConstant("glow", (float*)&layer, 1);
		layer = 2;
		services->setPixelShaderConstant("color", (float*)&layer, 1);
		services->setPixelShaderConstant("sunpos", (float*)&m_sky->m_sunpos, 3);
		v3f vector;
		vector.X=0;
		vector.Y=1;
		vector.Z=0;
		services->setPixelShaderConstant("vector", (float*)&vector, 3);
...
)
{
	...
	*/
	...
	Sky *sky = NULL;
	sky = new Sky(smgr->getRootSceneNode(), smgr, -1, &client);
